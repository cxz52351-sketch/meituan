...
import { addFeedback, getFeedbackStats } from "../services/feedback";
...
function computeSmartPicks(
  ...
  // 同校评论活跃度
  const recentReviews = getLatestReviews(
    50,
    university === "all" ? undefined : university,
  );
  const reviewCountByRestaurant: Record<string, number> = {};
  for (const r of recentReviews) {
    reviewCountByRestaurant[r.restaurantId] =
      (reviewCountByRestaurant[r.restaurantId] || 0) + 1;
  }
  ...
  const scored = pool.map((r) => {
    ...
    if (profile?.diningTags) {
      const tagMatches = profile.diningTags.filter(
        (tag) =>
          r.tags.some((rt) => rt.includes(tag) || tag.includes(rt)) ||
          r.category.includes(tag) ||
          tag.includes(r.category),
      ).length;
      taste += Math.min(tagMatches * 3, 7);
    }
    if (!hasHistory && !profile) taste = 12.5; // 中性分
    taste = Math.min(taste, 25);
    if (taste > bestSignalScore && taste > 12.5) {
      bestSignalScore = taste;
      bestSignal = "taste";
    }
    ...
    if (price > bestSignalScore * (15 / 25) && avgHistoryPrice > 0) {
      bestSignalScore = price * (25 / 15);
      bestSignal = "price";
    }

    // 4. 美团数据（15分满）: repurchaseRate + weeklyStudentOrders
    let platform =
      r.repurchaseRate * 10 + Math.min(r.weeklyStudentOrders / 100, 5);
    platform = Math.min(platform, 15);
    if (platform > bestSignalScore * (15 / 25)) {
      bestSignalScore = platform * (25 / 15);
      bestSignal = "platform";
    }
    ...
    if (dna) {
      ...
      let match = 0;
      // 辣味DNA高 → 加分火锅/烧烤
      if (spice >= 60 && ["火锅", "烧烤", "中餐"].includes(r.category))
        match += 3;
      // 省钱DNA高 → 加分低价
      ...
      if (
        socialDim >= 50 &&
        r.scenes.some((s) => ["聚餐", "和室友"].includes(s))
      )
        match += 2;
      // 探店DNA高 → 未去过的加分
    ...
    if (dnaScore > bestSignalScore * (10 / 25) && dna) {
      bestSignalScore = dnaScore * (25 / 10);
      bestSignal = "dna";
    }
    ...
    if (social > bestSignalScore * (10 / 25)) {
      bestSignalScore = social * (25 / 10);
      bestSignal = "social";
    }
    ...
    let feedbackAdj = 0;
    // 被拒绝的具体餐厅降权
    if (fbStats.rejectedRestaurantIds.has(r.id)) feedbackAdj -= 15;
    // 被拒品类降权
    if (fbStats.rejectedCategories[r.category]) {
      feedbackAdj -= Math.min(fbStats.rejectedCategories[r.category] * 3, 10);
    }
    // 因太贵被拒 → 同价位段降权
    if (
      fbStats.rejectedPriceThreshold > 0 &&
      r.avgPrice >= fbStats.rejectedPriceThreshold
    ) {
      feedbackAdj -= 5;
    }
    // 被采纳品类加权
    if (fbStats.acceptedCategories[r.category]) {
      feedbackAdj += Math.min(fbStats.acceptedCategories[r.category] * 2, 8);
    }
      ...
      base:
        r.distance <= 200
          ...
          : r.rating >= 4.5
            ? `${r.rating}分口碑好评`
            : r.actualPayPrice <= 15
              ? `实付仅¥${r.actualPayPrice}`
              : r.tags[0],
    };

    // 按维度得分排序，bestSignal优先
    const signalOrder: ReasonType[] = [
      bestSignal,
      "taste",
      "platform",
      "social",
...
export default function HomePage({ university }: Props) {
  ...
  // 反馈状态
  const [feedbackOpenId, setFeedbackOpenId] = useState<string | null>(null); // 展开拒绝原因的卡片ID
  ...
  const orderFeedPool = useMemo(() => {
    ...
    return filteredRestaurants.map((r) => {
      ...
      const dish =
        r.recommendDishes[Math.floor(Math.random() * r.recommendDishes.length)];
      return {
        id: r.id,
  ...
  const [flipParticipants, setFlipParticipants] = useState(
    todayFlip.participantBase,
  );
    ...
    <div className="page">
      ...
      >
        <div className="flip-hero-top">
          ...
          <span className="flip-hero-participants">
            {flipParticipants}人参与
          </span>
        </div>
        <div className="flip-hero-body">
          ...
          <div className="flip-hero-price">
            <span className="flip-price-original">
              ¥{todayFlip.originalPrice}
            </span>
        ...
        <div className="flip-hero-bottom" onClick={(e) => e.stopPropagation()}>
          <div className="flip-avatars">
            {"👦👧🧑👩🧑‍💻"
              .split(/(?=[\u{1F466}\u{1F467}\u{1F9D1}\u{1F469}])/u)
              .slice(0, 5)
              .map((a, i) => (
                <span key={i} className="flip-avatar">
                  {a}
                </span>
              ))}
          ...
          <div className="flip-hero-actions">
            ...
            {hasJoined && (
              <button
                ...
                onClick={() =>
                  navigate(`/restaurant/${todayFlip.restaurantId}`)
                }
              >
      ...
      {/* 明天翻牌投票 - 收起式入口 */}
      <div
        className="flip-vote-collapsed"
        onClick={() => setShowVoteExpand(!showVoteExpand)}
      >
        ...
        <span
          className={`flip-vote-collapsed-arrow ${showVoteExpand ? "expanded" : ""}`}
        >
          ›
        </span>
      </div>
        ...
        <div className="flip-vote">
          <div className="flip-vote-header">
            ...
            {tomorrowCandidates.length + userCandidates.length >= 5 ? (
              <span className="flip-vote-nominate-btn nominate-full">
                名额已满
              </span>
            ) : (
              <span
                className="flip-vote-nominate-btn"
          ...
          <div className="flip-vote-list">
            {[...tomorrowCandidates, ...userCandidates].slice(0, 5).map((c) => (
              ...
              >
                ...
                <span className="flip-vote-dish">
                  {c.dealDish} ¥{c.flipPrice}
                </span>
                <div className="flip-vote-bar-wrap">
                  <div
                    className="flip-vote-bar"
                    style={{
                      width: `${Math.min((c.votes + (votedId === c.restaurantId ? 1 : 0)) / 2.5, 100)}%`,
                    }}
                  />
                </div>
                <span className="flip-vote-count">
                  {c.votes + (votedId === c.restaurantId ? 1 : 0)}票
                </span>
              </div>
            ))}
          </div>
      ...
      {showNominate && (
        <div
          className="group-order-create-overlay"
          onClick={() => setShowNominate(false)}
        >
          <div
            className="group-order-create-modal flip-nominate-modal"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="group-order-create-top">
              <h3>提名明日翻牌餐厅</h3>
              <span
                className="group-order-create-close"
                onClick={() => setShowNominate(false)}
              >
                ✕
              </span>
            </div>
            <p className="group-order-create-hint">
              票数最高的餐厅将成为明日翻牌特价
            </p>
        ...
        <div className="ai-search-card" onClick={() => navigate("/ai")}>
          <div className="ai-search-card-left">
            ...
            <div className="ai-search-card-content">
              ...
              <div className="ai-search-card-desc">
                告诉我你的口味，帮你秒选
              </div>
            </div>
          </div>
            ...
            <>
              ...
              <div className="taste-dna-mini-labels">
                {tasteDNA.labels.slice(0, 3).map((label) => (
                  <span key={label} className="taste-dna-mini-chip">
                    {label}
                  </span>
                ))}
              </div>
      ...
      {orderFeedPool.length > 0 && (
        <div className="live-order-feed">
          ...
          <span className="live-time">
            {orderFeedPool[currentOrderIdx].time}
          </span>
        </div>
      )}
      ...
      <section className="section">
        ...
        <div className="smart-picks">
          {smartPicks.map(({ restaurant: r, reason }) => (
            <div
              key={r.id}
              className={`smart-pick-card${feedbackDoneIds.has(r.id) ? " feedback-done" : ""}`}
            >
              <div onClick={() => navigate(`/restaurant/${r.id}`)}>
                <img
                  className="smart-pick-img"
                  src={r.images[0]}
                  alt={r.name}
                  loading="lazy"
                />
              ...
              {!feedbackDoneIds.has(r.id) && (
                <div className="smart-pick-feedback">
                  <button
                    className="feedback-btn-reject"
                    onClick={(e) => {
                      e.stopPropagation();
                      setFeedbackOpenId(feedbackOpenId === r.id ? null : r.id);
                    }}
                  >
                    不想去
                  </button>
              ...
              {feedbackOpenId === r.id && (
                <div className="feedback-reject-options">
                  {(
                    [
                      ["reject_price", "太贵了"],
                      ["reject_distance", "太远了"],
                    ...
                    <button
                      key={action}
                      className="feedback-reject-option"
                      onClick={(e) => {
                        e.stopPropagation();
                        addFeedback({
                          restaurantId: r.id,
                          ...
                          category: r.category,
                          avgPrice: r.avgPrice,
                          source: "home_smart_pick",
                          action,
                          timestamp: Date.now(),
                        });
                        setFeedbackOpenId(null);
                        setFeedbackDoneIds((prev) => new Set(prev).add(r.id));
                        setFeedbackToast(
                          "记住了～团子继续努力，为你推荐更心仪的餐厅😊",
                        );
      ...
      <section className="section">
        ...
        <div className="quick-decisions">
          ...
          <button
            className="quick-decision-btn"
            onClick={() =>
              navigate("/ai", { state: { initialMessage: "深夜还有什么吃的" } })
            }
          >
            <span className="quick-decision-icon">🌙</span>
            <span>深夜觅食</span>
          ...
          <button
            className="quick-decision-btn"
            onClick={() =>
              navigate("/ai", { state: { initialMessage: "适合约会的餐厅" } })
            }
          >
            <span className="quick-decision-icon">💕</span>
            <span>约会推荐</span>
      ...
      <section className="section">
        ...
        <div className="rank-list-grid">
          {rankLists.slice(0, 4).map((rank) => (
            ...
            >
              <div className="icon" style={{ background: rank.color }}>
                {rank.icon}
              </div>
